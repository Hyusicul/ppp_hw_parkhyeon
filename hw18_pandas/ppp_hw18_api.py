...
def main():
    ...
    ans_2012=round(df_2012["rainfall"].sum(),1)

    df_2024=pd.read_csv(max_t_2024, skipinitialspace=True)
    ans_2024=round(df_2024["tmax"].max(),1)
  
    df_2020=pd.read_csv(max_tx_tm, skipinitialspace=True)
    ans_2020=round(df_2020["tdiff"].max(),1)
    ...
    sw2019=round(df_2019_sw["rainfall"].sum())
    jj2019=round(df_2019_jj["rainfall"].sum())
    ans_s_j=round(abs(sw2019-jj2019),1)
